...
//         if (memcmp(target->eeprom[blockNum], check, 4) == 0)
//             return;
//         nfc_reselect(nfc);
//     }
// }
...
//     uint32_t otp = (((uint32_t)(0xFFu - key->srix4k->eeprom[0x06][3]) << 24) | ((uint32_t)(0xFFu -
//     key->srix4k->eeprom[0x06][2]) << 16) |
//                     ((uint32_t)(0xFFu - key->srix4k->eeprom[0x06][1]) << 8) | (uint32_t)(0xFFu -
//                     key->srix4k->eeprom[0x06][0])) + 1u;
...
//     uint32_t masterKey = (uint32_t)(key->srix4k->uid * (uint64_t)((((uint32_t)b18[2] << 24) |
//     ((uint32_t)b18[3] << 16) | ((uint32_t)b19[2] << 8) | (uint32_t)b19[3]) + 1u));
...
//     ((uint64_t)uid_bytes[5] << 40) |
//                        ((uint64_t)uid_bytes[4] << 32) | ((uint64_t)uid_bytes[3] << 24) |
//                        ((uint64_t)uid_bytes[2] << 16) |
...
//     key->srix4k->eeprom[0x18][2] == 0x0F &&
//            key->srix4k->eeprom[0x18][3] == 0x48 && key->srix4k->eeprom[0x19][0] == 0xC0 &&
//            key->srix4k->eeprom[0x19][1] == 0x82 && key->srix4k->eeprom[0x19][2] == 0x00 &&
//            key->srix4k->eeprom[0x19][3] == 0x07;
// }
...
//     int yy = (key->srix4k->eeprom[0x08][3] & 0x0F) * 1000 + ((key->srix4k->eeprom[0x08][3] & 0xF0) >> 4) *
//     100 +
...
//         month += 12;
//     }
//     return (uint32_t)(year * 365 + year / 4 - year / 100 + year / 400 + (month * 153 + 3) / 5 + day -
//     728692);
// }
...
//                 int dd = ((key->srix4k->eeprom[0x08][0] & 0xF0) >> 4) * 10 + (key->srix4k->eeprom[0x08][0]
//                 & 0x0F); int mm = ((key->srix4k->eeprom[0x08][1] & 0xF0) >> 4) * 10 +
//                 (key->srix4k->eeprom[0x08][1] & 0x0F); int yy = (key->srix4k->eeprom[0x08][3] & 0x0F) *
//                 1000 + ((key->srix4k->eeprom[0x08][3] & 0xF0) >> 4) * 100 +
//                          ((key->srix4k->eeprom[0x08][2] & 0xF0) >> 4) * 10 + (key->srix4k->eeprom[0x08][2]
//                          & 0x0F);
...
//                 srix_flag_add(&target->srixFlag, b);
//         } else {
//             return -1;
//         }
//     }
